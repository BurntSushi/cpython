...
class TestDescriptions(unittest.TestCase):
    ...
    def test_module_level_callable_noargs(self):
        self.assertEqual(self._get_summary_line(time.time), "time()")
    ...
    def test_module_level_callable_o(self):
        ...
        self.assertEqual(self._get_summary_line(_stat.S_IMODE), "S_IMODE(object, /)")

    def test_unbound_builtin_method_noargs(self):
        self.assertEqual(self._get_summary_line(str.lower), "lower(self, /)")
    ...
    def test_unbound_builtin_method_o(self):
        self.assertEqual(self._get_summary_line(set.add), "add(self, object, /)")

    def test_bound_builtin_method_o(self):
        self.assertEqual(
            self._get_summary_line(set().add),
            "add(object, /) method of builtins.set instance",
        )

    def test_unbound_builtin_method_coexist_o(self):
        self.assertEqual(
            self._get_summary_line(set.__contains__), "__contains__(self, object, /)"
        )
    ...
    def test_unbound_builtin_classmethod_noargs(self):
        self.assertEqual(
            self._get_summary_line(datetime.datetime.__dict__["utcnow"]),
            "utcnow(type, /)",
        )

    def test_bound_builtin_classmethod_noargs(self):
        self.assertEqual(
            self._get_summary_line(datetime.datetime.utcnow),
    ...
    def test_bound_builtin_classmethod_o(self):
        self.assertEqual(
            self._get_summary_line(dict.__class_getitem__),
    ...
    def test_module_level_callable_unrepresentable_default(self):
        ...
        self.assertEqual(
            self._get_summary_line(builtin),
    ...
    def test_builtin_staticmethod_unrepresentable_default(self):
        ...
        self.assertEqual(self._get_summary_line(cls.staticmeth), "staticmeth(a, b=<x>)")

    @support.cpython_only
    def test_unbound_builtin_method_unrepresentable_default(self):
        self.assertEqual(
            self._get_summary_line(dict.pop),
            "pop(self, key, default=<unrepresentable>, /)",
        )
        import _testcapi

        cls = _testcapi.DocStringUnrepresentableSignatureTest
        self.assertEqual(self._get_summary_line(cls.meth), "meth(self, /, a, b=<x>)")

    @support.cpython_only
    def test_bound_builtin_method_unrepresentable_default(self):
        self.assertEqual(
            self._get_summary_line({}.pop),
        ...
        self.assertEqual(
            self._get_summary_line(obj.meth),
            "meth(a, b=<x>) "
    ...
    def test_unbound_builtin_classmethod_unrepresentable_default(self):
        ...
        self.assertEqual(self._get_summary_line(descr), "classmeth(type, /, a, b=<x>)")
    ...
    def test_bound_builtin_classmethod_unrepresentable_default(self):
        ...
        cls = _testcapi.DocStringUnrepresentableSignatureTest
        self.assertEqual(
            self._get_summary_line(cls.classmeth),
    ...
    def test_overridden_text_signature(self):
        ...
        for text_signature, unbound, bound in [
            ...
            with self.subTest(text_signature):
                ...
                self.assertEqual(self._get_summary_line(C.meth), "meth" + unbound)
                self.assertEqual(
                ...
                self.assertEqual(
                    self._get_summary_line(C.cmeth),
                    "cmeth" + bound + " method of builtins.type instance",
                )
                C.smeth.__text_signature__ = text_signature
                self.assertEqual(self._get_summary_line(C.smeth), "smeth" + unbound)
