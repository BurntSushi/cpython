...
class ReTests(unittest.TestCase):
    ...
    def test_qualified_re_sub(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"sub\(\) got multiple values for argument 'count'"
        ):
            re.sub("a", "b", "aaaaa", 1, count=1)
        with self.assertRaisesRegex(
            TypeError, r"sub\(\) got multiple values for argument 'flags'"
        ):
            re.sub("a", "b", "aaaaa", 1, 0, flags=0)
        with self.assertRaisesRegex(
            TypeError,
            r"sub\(\) takes from 3 to 5 positional arguments but 6 " r"were given",
        ):
    ...
    def test_misuse_flags(self):
        ...
        self.assertEqual(str(w.warning), "'count' is passed as positional argument")
        self.assertEqual(w.filename, __file__)
        with self.assertWarns(DeprecationWarning) as w:
        ...
        self.assertEqual(str(w.warning), "'maxsplit' is passed as positional argument")
        self.assertEqual(w.filename, __file__)
    ...
    def test_re_subn(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"subn\(\) got multiple values for argument 'count'"
        ):
            re.subn("a", "b", "aaaaa", 1, count=1)
        with self.assertRaisesRegex(
            TypeError, r"subn\(\) got multiple values for argument 'flags'"
        ):
            re.subn("a", "b", "aaaaa", 1, 0, flags=0)
        with self.assertRaisesRegex(
            TypeError,
            r"subn\(\) takes from 3 to 5 positional arguments but 6 " r"were given",
        ):
    ...
    def test_qualified_re_split(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"split\(\) got multiple values for argument 'maxsplit'"
        ):
            re.split(":", ":a:b::c", 2, maxsplit=2)
        with self.assertRaisesRegex(
            TypeError, r"split\(\) got multiple values for argument 'flags'"
        ):
            re.split(":", ":a:b::c", 2, 0, flags=0)
        with self.assertRaisesRegex(
            TypeError,
            r"split\(\) takes from 2 to 4 positional arguments but 5 " r"were given",
        ):
