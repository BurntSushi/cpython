...
class TimerfdTests(unittest.TestCase):
    ...
    def test_timerfd_initval(self):
        ...
        # 1st call
        next_expiration, interval2 = os.timerfd_settime(
            fd, initial=initial_expiration, interval=interval
        )
            ...
            for flags in test_flags:
                with self.subTest(flags=flags, initial=initial, interval=interval):
                    with self.assertRaises(OSError) as context:
                        os.timerfd_settime(
                            fd, flags=flags, initial=initial, interval=interval
                        )
                    ...
                    with self.assertRaises(OSError) as context:
                        ...
                        os.timerfd_settime_ns(
                            fd, flags=flags, initial=initial_ns, interval=interval_ns
                        )
                    self.assertEqual(context.exception.errno, errno.EINVAL)
    ...
    def test_timerfd_select(self):
        ...
        for _ in range(count):
            rfd, wfd, xfd = select.select(
                [fd], [fd], [fd], initial_expiration + interval
            )
            self.assertEqual((rfd, wfd, xfd), ([fd], [], []))
            self.assertEqual(self.read_count_signaled(fd), 1)
        t = time.perf_counter() - t
    ...
    def check_timerfd_poll(self, nanoseconds):
        ...
        if nanoseconds:
            os.timerfd_settime_ns(
                fd, initial=initial_expiration_ns, interval=interval_ns
            )
        else:
            os.timerfd_settime(
                fd,
    ...
    def test_timerfd_ns_initval(self):
        ...
        next_expiration_ns, interval_ns2 = os.timerfd_settime_ns(
            fd, initial=initial_expiration_ns, interval=interval_ns
        )
        ...
        self.assertAlmostEqual(
            next_expiration_ns, initial_expiration_ns, delta=limit_error
        )
    ...
    def test_timerfd_ns_select(self):
        ...
        for _ in range(count):
            rfd, wfd, xfd = select.select(
                [fd], [fd], [fd], (initial_expiration_ns + interval_ns) / 1e9
            )
            self.assertEqual((rfd, wfd, xfd), ([fd], [], []))
            self.assertEqual(self.read_count_signaled(fd), 1)
        t = time.perf_counter_ns() - t
...
class CPUCountTests(unittest.TestCase):
    ...
    @unittest.skipUnless(
        hasattr(os, "sched_setaffinity"), "don't have sched affinity support"
    )
    def test_process_cpu_count_affinity(self):
        ...
        if len(mask) <= 1:
            self.skipTest(
                f"sched_getaffinity() returns less than " f"2 CPUs: {sorted(mask)}"
            )
...
class TestPEP519(unittest.TestCase):
    ...
    def test_fspath_set_to_None(self):
        ...
        good_error_msg = r"expected str, bytes or os.PathLike object, not {}".format
        ...
        other_good_error_msg = r"should be string, bytes or os.PathLike, not {}".format
