...
def reraise_if_not_enum(*enum_types_or_exceptions):
    ...
    def decorator(func):
        ...
        def inner(*args, **kwargs):
            excs = [e for e in enum_types_or_exceptions if isinstance(e, Exception)]
            if len(excs) == 1:
                raise excs[0]
            elif excs:
...
class TestSpecial(unittest.TestCase):
    ...
    def test_custom_strenum(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"argument (2|'encoding') must be str, not "
        ):
        ...
        with self.assertRaisesRegex(
            TypeError, r"argument (3|'errors') must be str, not "
        ):
