...
class TestTokenize(TestCase):
    ...
    def assertExactTypeEqual(self, opstr, *optypes):
        ...
        self.assertEqual(
            tokenize.tok_name[tokens[0].exact_type],
            tokenize.tok_name[tokenize.ENCODING],
        )
        for i in range(num_optypes):
            self.assertEqual(
                tokenize.tok_name[tokens[i + 1].exact_type],
                tokenize.tok_name[optypes[i]],
            )
        self.assertEqual(
            tokenize.tok_name[tokens[1 + num_optypes].exact_type],
            tokenize.tok_name[token.NEWLINE],
        )
        self.assertEqual(
            tokenize.tok_name[tokens[2 + num_optypes].exact_type],
    ...
    def test_comment_at_the_end_of_the_source_without_newline(self):
        ...
        expected_tokens = [
            ...
            tokenize.TokenInfo(
                type=token.NL, string="", start=(3, 5), end=(3, 6), line="#test"
            ),
            tokenize.TokenInfo(
    ...
    def test_newline_and_space_at_the_end_of_the_source_without_newline(self):
        ...
        expected_tokens = [
            ...
            tokenize.TokenInfo(
                token.NAME, string="a", start=(1, 0), end=(1, 1), line="a\n"
            ),
            tokenize.TokenInfo(
            ...
            tokenize.TokenInfo(
                token.ENDMARKER, string="", start=(3, 0), end=(3, 0), line=""
            ),
        ]
...
class TestRoundtrip(TestCase):
    ...
    def roundtrip(self, code):
        ...
        return tokenize.untokenize(tokenize.tokenize(BytesIO(code).readline)).decode(
            "utf-8"
        )
...
class CTokenizeTest(TestCase):
    ...
    def test_encoding(self):
        ...
        expected = [
            tokenize.TokenInfo(
                type=tokenize.NUMBER, string="1", start=(1, 0), end=(1, 1), line="1+1"
            ),
            tokenize.TokenInfo(
                type=tokenize.OP, string="+", start=(1, 1), end=(1, 2), line="1+1"
            ),
            tokenize.TokenInfo(
                type=tokenize.NUMBER, string="1", start=(1, 2), end=(1, 3), line="1+1"
            ),
            tokenize.TokenInfo(
                type=tokenize.NEWLINE, string="", start=(1, 3), end=(1, 4), line="1+1"
            ),
            tokenize.TokenInfo(
                type=tokenize.ENDMARKER, string="", start=(2, 0), end=(2, 0), line=""
            ),
        ]
  ...
  await = 2""",
    ...
    def test_max_indent(self):
        ...
        self.assertRaises(
            IndentationError,
            lambda: list(
                tokenize._generate_tokens_from_c_tokenizer(the_input.readline)
            ),
        )
    ...
    def test_continuation_lines_indentation(self):
        def get_tokens(string):
            ...
            return [
                (kind, string)
                for (kind, string, *_) in tokenize._generate_tokens_from_c_tokenizer(
                    the_string.readline
                )
            ]
