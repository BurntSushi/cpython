...
class CFunctionCallsErrorMessages(unittest.TestCase):
    ...
    def test_varargs1min(self):
        msg = (
            r"get\(\) takes at least 1 argument \(0 given\)|"
    ...
    def test_varargs1max(self):
        msg = (
            r"input\(\) takes at most 1 argument \(2 given\)|"
    ...
    def test_varargs2max(self):
        msg = (
            r"get\(\) takes at most 2 arguments \(3 given\)|"
    ...
    def test_unexpected_keyword_suggestion_valid_positions(self):
        ...
        for keyword, suggestion in cases:
            with self.subTest(keyword):
                ctx = (
                    self.check_suggestion_includes(suggestion)
                    if suggestion
                    else self.check_suggestion_not_pressent()
                )
                with ctx:
                    foo(**{keyword: None})
