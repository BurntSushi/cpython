...
class CAPITest(unittest.TestCase):
    ...
    def test_object_hasattr(self):
        ...
        with support.catch_unraisable_exception() as cm:
            ...
            self.assertEqual(str(cm.unraisable.exc_value), "do not get evil")
    ...
    def test_object_hasattrstring(self):
        ...
        with support.catch_unraisable_exception() as cm:
            ...
            self.assertRegex(str(cm.unraisable.exc_value), "'utf-8' codec can't decode")
    ...
    def test_mapping_haskey(self):
        ...
        with support.catch_unraisable_exception() as cm:
            ...
            self.assertEqual(cm.unraisable.exc_type, TypeError)
            self.assertEqual(
                str(cm.unraisable.exc_value), "'int' object is not subscriptable"
            )

        with support.catch_unraisable_exception() as cm:
            self.assertFalse(haskey({}, []))
            self.assertEqual(cm.unraisable.exc_type, TypeError)
            self.assertEqual(str(cm.unraisable.exc_value), "unhashable type: 'list'")

        with support.catch_unraisable_exception() as cm:
            self.assertFalse(haskey([], 1))
            self.assertEqual(cm.unraisable.exc_type, IndexError)
            self.assertEqual(str(cm.unraisable.exc_value), "list index out of range")
        ...
        with support.catch_unraisable_exception() as cm:
            ...
            self.assertEqual(
                str(cm.unraisable.exc_value), "null argument to internal routine"
            )
