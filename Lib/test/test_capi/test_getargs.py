    ...
    def test_nonascii_keywords(self):
        ...
        for name in ("a", "ä", "ŷ", "㷷", "𐀀"):
            with self.subTest(name=name):
                ...
                with self.assertRaisesRegex(
                    TypeError, f"function missing required argument '{name}'"
                ):
                    parse((), {}, "O", [name])
                with self.assertRaisesRegex(
                    TypeError,
                    rf"argument for function given by name \('{name}'\) "
                    rf"and position \(1\)",
                ):
                    parse((1,), {name: 2}, "O|O", [name, "b"])
                with self.assertRaisesRegex(
                    TypeError, f"'{name}' is an invalid keyword argument"
                ):
                    parse((), {name: 1}, "|O", ["b"])
                with self.assertRaisesRegex(
                    TypeError, "'b' is an invalid keyword argument"
                ):
                    parse((), {"b": 1}, "|O", [name])

                invalid = name.encode() + (name.encode()[:-1] or b"\x80")
                self.assertEqual(parse((), {}, "|O", [invalid]), (NULL,))
                self.assertEqual(parse((1,), {"b": 2}, "O|O", [invalid, "b"]), (1, 2))
                with self.assertRaisesRegex(
                    TypeError, f"function missing required argument '{name}\ufffd'"
                ):
                    parse((), {}, "O", [invalid])
                with self.assertRaisesRegex(
                    UnicodeDecodeError, f"'utf-8' codec can't decode bytes? "
                ):
                ...
                for name2 in ("b", "ë", "ĉ", "Ɐ", "𐀁"):
                    with self.subTest(name2=name2):
                        with self.assertRaisesRegex(
                            TypeError, f"'{name2}' is an invalid keyword argument"
                        ):
