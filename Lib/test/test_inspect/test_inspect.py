...
def meth_self_noargs(self, /):
    pass


def meth_self_o(self, object, /):
    pass


def meth_type_noargs(type, /):
    pass


def meth_type_o(type, object, /):
    pass
...
class TestBuggyCases(GetSourceBase):
    ...
    def test_class_with_method_from_other_module(self):
        with tempfile.TemporaryDirectory() as tempdir:
            with open(
                os.path.join(tempdir, "inspect_actual%spy" % os.extsep),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                os.path.join(tempdir, "inspect_other%spy" % os.extsep),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
    ...
    def test_getfullargspec_builtin_func_no_signature(self):
        ...
        for builtin, template in tests:
            with self.subTest(builtin):
                self.assertEqual(
                    inspect.getfullargspec(builtin), inspect.getfullargspec(template)
                )
...
class TestSignatureObject(unittest.TestCase):
    ...
    def test_signature_on_builtins_no_signature(self):
        ...
        for builtin, template in tests:
            with self.subTest(builtin):
                self.assertEqual(
                    inspect.signature(builtin), inspect.signature(template)
                )
...
class TestRepl(unittest.TestCase):
    ...
    def test_getsource(self):
        output = self.run_on_interactive_mode(
            textwrap.dedent(
                """\
        def f():
            print(0)
