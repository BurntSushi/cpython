...
class PurePathTest(unittest.TestCase):
    ...
    def test_with_suffix_common(self):
        ...
        self.assertRaises(ValueError, P("a/b").with_suffix, ".d/.")
        self.assertRaises(ValueError, P("a/b").with_suffix, (self.pathmod.sep, "d"))
        ...
        try:
            ...
            if not parents:
                raise FileNotFoundError(
                    errno.ENOENT, "File not found", str(self.parent)
                ) from None
...
class DummyPathTest(unittest.TestCase):
    ...
    def test_read_text_with_newlines(self):
        ...
        self.assertEqual(
            (p / "fileA").read_text(newline="\n"), "abcde\r\nfghlk\n\rmnopq"
        )
        # Check that `\r` character replaces `\n`
        (p / "fileA").write_bytes(b"abcde\r\nfghlk\n\rmnopq")
        self.assertEqual(
            (p / "fileA").read_text(newline="\r"), "abcde\r\nfghlk\n\rmnopq"
        )
        # Check that `\r\n` character replaces `\n`
        (p / "fileA").write_bytes(b"abcde\r\nfghlk\n\rmnopq")
        self.assertEqual(
    ...
    def test_glob_follow_symlinks_common(self):
        ...
        def _check(path, glob, expected):
            actual = {
                path
                for path in path.glob(glob, follow_symlinks=True)
                if "linkD" not in path.parent.parts
            }  # exclude symlink loop.
        ...
        _check(
            p,
            "dir*/*/**/",
            ["dirA/linkC", "dirA/linkC/linkD", "dirB/linkD", "dirC/dirD"],
        )
    ...
    def test_rglob_follow_symlinks_common(self):
        ...
        def _check(path, glob, expected):
            actual = {
                path
                for path in path.rglob(glob, follow_symlinks=True)
                if "linkD" not in path.parent.parts
            }  # exclude symlink loop.
            self.assertEqual(actual, {P(BASE, q) for q in expected})
        ...
        p = P(BASE, "dirC")
        _check(p, "*", ["dirC/fileC", "dirC/novel.txt", "dirC/dirD", "dirC/dirD/fileD"])
    ...
    def _check_complex_symlinks(self, link0_target):
        ...
        P = self.cls(BASE)
        P.joinpath("link1").symlink_to(
            os.path.join("link0", "link0"), target_is_directory=True
        )
        P.joinpath("link2").symlink_to(
            os.path.join("link1", "link1"), target_is_directory=True
        )
        P.joinpath("link3").symlink_to(
            os.path.join("link2", "link2"), target_is_directory=True
        )
    ...
    def setUpWalk(self):
        ...
        if self.can_symlink:
            ...
            self.sub2_tree = (
                self.sub2_path,
                [],
                ["broken_link", "broken_link2", "link", "tmp3"],
            )
        else:
...
class PosixPathTest(PathTest, PurePosixPathTest):
    ...
    def test_from_uri_pathname2url(self):
        ...
        self.assertEqual(
            P.from_uri("file:" + pathname2url("//foo/bar")), P("//foo/bar")
        )
...
class WindowsPathTest(PathTest, PureWindowsPathTest):
    ...
    def test_from_uri(self):
        ...
        # Localhost paths
        self.assertEqual(
            P.from_uri("file://localhost/c:/path/to/file"), P("c:/path/to/file")
        )
        self.assertEqual(
            P.from_uri("file://localhost/c|/path/to/file"), P("c:/path/to/file")
        )
    ...
    def test_from_uri_pathname2url(self):
        P = self.cls
        self.assertEqual(
            P.from_uri("file:" + pathname2url(r"c:\path\to\file")), P("c:/path/to/file")
        )
        self.assertEqual(
            P.from_uri("file:" + pathname2url(r"\\server\path\to\file")),
            P("//server/path/to/file"),
        )
