...
_Instruction.line_number.__doc__ = (
...
class Instruction(_Instruction):
    ...
    def _get_argval_argrepr(op, arg, offset, co_consts, names, varname_from_oparg):
        ...
        if arg is not None:
            ...
            if deop in hasconst:
                ...
                argrepr = ", ".join(
                    s for i, s in enumerate(FUNCTION_ATTR_FLAGS) if arg & (1 << i)
                )
            elif deop == BINARY_OP:
    ...
    def _create(
        ...
        argval, argrepr = cls._get_argval_argrepr(
            op, arg, offset, co_consts, names, varname_from_oparg
        )
        return Instruction(
            _all_opname[op],
            op,
    ...
    for instr in _get_instructions_bytes(
        ...
        if show_caches:
            ...
            is_current_instr = (
                ...
                <= lasti
                <= instr.offset
                + 2 * _get_cache_size(_all_opname[_deoptop(instr.opcode)])
            )
        print(
...
def _unpack_opargs(code):
    ...
    extended_args_offset = (
        0  # Number of EXTENDED_ARG instructions preceding the current instruction
    )
    caches = 0
...
def _find_imports(co):
    ...
    names = co.co_names
    opargs = [
        (op, arg) for _, _, op, arg in _unpack_opargs(co.co_code) if op != EXTENDED_ARG
    ]
...
def main():
    ...
    parser.add_argument(
        "-C", "--show-caches", action="store_true", help="show inline caches"
    )
